...
library PoolMath {
    ...
    /// @dev Formula: `E = e^rt`
    /// @return exchangeRate the exchange rate in terms of underlying. Guaranteed to be positive or zero.
    ...
    /// @return netUnderlyingToAccount the amount of underlying the swapper will receive
    /// negative if the swapper is selling BaseLpt and positive if the swapper is buying BaseLpt.
    /// @return underlyingFee the amount of underlying charged as swap fee
    /// this includes `underlyingToProtocol`
    /// @return underlyingToProtocol the amount of underlying the Pool fee recipient will receive as fee
    ...
    function calculateSwap(
        ...
        //                                 netBaseLptToAccount
        // netUnderlyingToAccount = -1 * ────────────────────────
        //                                       extRate
        // where `netBaseLptToAccount` is the delta of baseLpt (`d_pt`) and `netBaseLptToAccount` is the delta of underlying (`d_u`).
        ...
        int256 netUnderlying = netBaseLptToAccount.divWadDown(preFeeExchangeRate).neg();
        ...
        if (netBaseLptToAccount > 0) {
            ...
            //     = netUnderlying * (feeRate - 1)
            underlyingFee = netUnderlying.mulWadDown(SignedMath.WAD - comp.feeRate);
        } else {
            ...
            underlyingFee = ((netUnderlying * (SignedMath.WAD - comp.feeRate)) / comp.feeRate).neg();
        }

        // Subtract swap fee
        // underlyingWithFee = underlyingNoFee - fee
        int256 netUnderlyingToAccount = netUnderlying - underlyingFee;
        // Charge protocol fee on swap fee
        // This underlying will be removed from the pool reserve
        int256 underlyingToProtocol = (underlyingFee * pool.protocolFeePercent.toInt256()) / FULL_PERCENTAGE;

        return (netUnderlyingToAccount, underlyingFee, underlyingToProtocol);
    }
    ...
    /// @param netUnderlyingToAccount net underlying to account. positive if the swapper is selling BaseLpt.
    /// @param netUnderlyingToProtocol should be removed from the pool reserve `totalUnderlying18`. must be positive
    function _setPostPoolState(
        ...
        pool.totalUnderlying18 =
            (pool.totalUnderlying18).toInt256().subNoNeg(netUnderlyingToAccount + netUnderlyingToProtocol).toUint256();
    ...
    /// @return exchangeRate the exchange rate in terms of underlying
